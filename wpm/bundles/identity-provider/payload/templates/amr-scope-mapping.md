...
out of the box. GLA's adapter reads `amr`/`acr` to derive `AuthStrength`:

- **Without** this mapping → `amr` is empty, GLA **floors every login to `password`** and **never up-maps** (the
  safe failure). The integration runs as **password-only**: a passkey login cannot reach `webauthn` strength, so
  a route requiring `requiredAuthStrength: "webauthn"` would reject even a real passkey. **Warn the operator.**
- **With** this mapping → a password login emits `amr: ["pwd"]` → `password`; a passkey login emits
  `amr: ["swk"]` → `webauthn`. The dual-method strength gate then works as designed.
...
recorded, then **attached to GLA's OAuth2 provider's property mappings** (alongside the standard `openid`/`profile`
scopes).
...
3. **Attach** the returned `pk` to GLA's OAuth2 provider's `property_mappings` (merge with the standard scopes;
   PATCH the provider).
4. Verify the emitted `amr` end-to-end: drive a real password login → the id_token carries `amr: ["pwd"]`; a real
   passkey login → `amr: ["swk"]`.

## Adapter alignment — no adapter change

GLA's default amr map (`adapters/auth-authentik/src/strength.ts` `DEFAULT_METHOD_MAPS`) is
`{hwk, swk, webauthn, fido} → webauthn` and `{pwd} → password` — **exactly** what this expression emits, so
`GLA_AUTHENTIK_AMR_MAP` needs **no override** for this recipe. The adapter already handled both the empty-`amr`
floor and the explicit `["pwd"]` correctly (proven in the rehearsal); only authentik's flow config needed this.
...
`{ "amr": ["pwd"], "acr": "goauthentik.io/providers/oauth2/default", "gla_auth_method": "password", "sub":
"6bd4…9a33" }` → GLA `verifyAssertion` → `{ ok: true, authStrength: "password" }` with `methodResolvable: true`.
The passkey arm (`amr: ["swk"] → webauthn`) is the same mechanism; its real round-trip is the deploy's job
