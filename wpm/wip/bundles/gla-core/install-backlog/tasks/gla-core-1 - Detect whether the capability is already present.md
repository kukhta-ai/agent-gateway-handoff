...
Reason about each prerequisite by inspection (do not run setup): **Node** — GLA's workspace declares engines.node >= 22, so treat Node 22 as the floor; a newer Node already present is adopted, not replaced, and only an absent/too-old Node needs installing (the validated acquisition path on a Debian/Ubuntu host is NodeSource `setup_22.x` then the distro's nodejs package — but that is a setup choice to record now, not run here). **pnpm** — GLA is a pnpm workspace built with `pnpm install` + `pnpm build`; the validated path obtains it from the Node that ships corepack (`corepack enable && corepack prepare pnpm@<pinned> --activate`) rather than a global npm install, so an absent pnpm is a resolvable prerequisite, not a blocker; record how it would be obtained. **Build headroom** — a from-source build of the workspace needs roughly 2 GB of free disk (node_modules + the compiled output); determine the free space on the target filesystem so a build does not fail part-way for lack of room. **A service supervisor** — prefer user-scope systemd for least privilege (the bridge socket then lands at /run/user/<uid>/gla.sock and the unit needs lingering enabled to survive logout); system-scope systemd is the alternative; a documented foreground run command is the fallback when neither is usable. **Any existing GLA runtime** — a prior build tree or an enabled gla service must be reconciled, not duplicated.
...
- [ ] #5 whether the target filesystem has enough free space for a from-source build (on the order of 2 GB for dependencies plus compiled output) is determined, so an out-of-disk failure mid-build is anticipated rather than discovered
- [ ] #6 each prerequisite's presence and version, and whether it was already on the host, are recorded for the receipt before setup runs
<!-- AC:END -->
