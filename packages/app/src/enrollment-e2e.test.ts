 ...
 */
async function startStack(): Promise<{ stack: EnrollmentStack; origin: string }> {
...
describe("REAL WebAuthn enrollment end-to-end (virtual authenticator; GLA-013 AC#1/#3)", () => {
  it.runIf(HAVE_CHROMIUM)(
    ...
    async () => {
      ...
      const { stack, origin } = await startStack();
      ...
      expect(invite.link).toContain("/enroll?grant=");
      const link = invite.link.replace("127.0.0.1", "localhost");
...
describe("REAL gateway — un-enrolled cannot authenticate; a grant is bound to its recipient (GLA-013 AC#2/#3)", () => {
  it.runIf(HAVE_CHROMIUM)(
    ...
    async () => {
      ...
      const otherInvite = await stack.enrollInvite(otherRecipient);
      const getRes = await fetch(otherInvite.link.replace("127.0.0.1", "localhost"));
