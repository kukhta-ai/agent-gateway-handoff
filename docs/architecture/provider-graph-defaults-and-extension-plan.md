...
> **Scope:** generalize "a default package for every layer" into a typed provider graph, split deployment/capsule
> selection, and a repeatable provider-author workflow. This sharpens `docs/02-provider-and-extension-model.md`,
> `docs/architecture/provider-layer-refactor-contract.md`, and
> `docs/architecture/provider-host-extension-architecture.md`; it does not introduce dynamic hot-loading, a public
> plugin ABI, marketplace resolution, or runtime-agent code registration. Capsule templates participate in the graph
> as catalog packages, not executable ProviderRegistry factories.
...
for a layer horizontally: add the provider package, satisfy any dependencies, select app-infrastructure providers in
`AppDeploymentConfig` or capsule providers in template defaults, and use the provider through the catalog without
editing the kernel, gateway, session, identity, worker core, or generic app composition.

The design target is therefore:

- defaults are ordinary providers, not special cases;
- app infrastructure selection is deployment config and capsule selection is template/spec composition, not app code;
...
Represent the extension surface as a **typed provider graph** derived from `ProviderRegistry`, template packages,
deployment config, WPM evidence, probes, and Catalog state.

The graph is not a required graph database. It is the architectural model and validation shape behind the existing
runtime objects: provider modules, manifests, app deployment config, template defaults, dependency requirements, WPM
bindings, probes, skills, and runtime factories. Each runtime flow consumes a projection of the graph:

- Catalog projects discovery, schemas, skills, compatibility, diagnostics, and availability.
- Admission Resolver projects a candidate `AssemblySpec` and rejects unavailable or incompatible nodes before session
  creation.
- ProviderRegistry and its internal factory runner project selected provider factories and create runtime ports.
...
> App-infrastructure defaults are selected by `AppDeploymentConfig`. Capsule defaults are selected by
> `CapsuleTemplate` / `TemplatePackage` data, and runtime deltas are `AssemblySpec` proposals over those registered
> providers.
...
| `TemplatePackage` | A trusted catalog package for `CapsuleTemplate` entities, schemas, defaults, skills/docs, and tests. It has no executable ProviderRegistry factory unless it also ships runtime providers. |
...
| `ProviderModule` | Executable trusted code registered into `ProviderRegistry`; current implementations may run factories through internal ProviderHost mechanics. |
| `ProviderRegistry` | Boot-time executable registry for trusted provider packages, provider ids, factories, probes, schemas, assets, and state namespaces. |
| `AppDeploymentConfig` | Operator/app boot selection and config for app infrastructure providers: auth, channel, and secret store. |
| `CapsuleDefault` | A template-owned fixed/default/open capsule provider selection for launcher, workspace, entrypoint, connector, or detector. |
| `AssemblySpec` | Runtime-agent session proposal containing only template-open capsule deltas and provider-owned params. |
| `CapabilityCatalog` | Read-only projection of registry, templates, deployment config, WPM evidence, probes, skills, schemas, assets, and diagnostics. |
| `Admission Resolver` | Proposal-to-plan resolver that checks an `AssemblySpec` against catalog capability, compatibility, evidence, policy, and provider config before provisioning. |
...
| `selectedBy` | `AppDeploymentConfig` or `CapsuleTemplate` selects a provider id in its owned family set. |
...
| `suppliesAsset` | A provider contributes reviewed client assets projected by catalog and mounted by generic gateway code. |
...
- `ProviderManifest`;
- ProviderRegistry module/factory registration, with internal ProviderHost mechanics allowed only behind that boundary;
...
Defaults are selected only by explicit distribution/deployment inputs. Generic app composition receives a sealed
`ProviderRegistry`, `AppDeploymentConfig`, template defaults, probe maps, and client asset evidence from the
distribution entrypoint; it does not know which provider is "the" default.

A reference distribution may group default providers for convenience, but it is not a monolithic host installer or a
target `ProviderSet` entity. Host-touching dependencies remain separate WPM bundles and become available only
through structured WPM receipts plus current GLA probes. Optional families may use an explicit disabled/null provider
only where "absent" is a valid product behavior; security-critical layers must have a real default or fail closed.
...
contract tests. It is selected and validated through Catalog/Admission Resolver, but ProviderRegistry/internal factory
mechanics do not instantiate a long-lived template factory.

Default means **selected by a named deployment/template preset**, not globally preferred for all deployments.
Compatibility decides whether a selected set of providers can safely work together; it does not decide which product
posture the operator intended. The reference distribution may therefore expose multiple named presets:

| Preset | Purpose | Selection posture |
|---|---|---|
...
During migration these names may still be read from provider-profile manifests, but the target rule is fixed:
documentation and implementation must say which deployment/template preset a "default" belongs to. If two docs
disagree about WebAuthn vs authentik or process launcher vs Docker, that is not solved by a generic default; it is
solved by naming the preset and then checking compatibility and availability inside that selected graph.
...
- the ProviderRegistry registration method and factory context;
...
| 2 | `AuthProvider` | in-tree WebAuthn/passkey verification for the simple deployment preset | authentik, generic OIDC, Authelia, hosted IdP adapters. |
...
## 6. Deployment Config And Template Defaults

The target model splits the old broad profile into two owned selection surfaces:

- `AppDeploymentConfig` names `AuthProvider`, `ChannelAdapter`, and `SecretStore`, plus provider-owned config for
  those app infrastructure providers.
- `TemplatePackage` / `CapsuleTemplate` defaults name `Launcher`, `Workspace`, `HumanEntrypoint`,
  `AgentConnector`, and `CompletionDetector`. `AssemblySpec` may override only template-open capsule parts.

Those documents are **operator-owned install/update artifacts** or trusted template package content. They are loaded
at daemon boot. Changing them requires the operator install/update flow and daemon restart. A future hot-reload path
may exist only if it uses the same checks, validation pipeline, and fail-closed rollback. Runtime request payloads,
agent CLI calls, and catalog reads cannot write deployment config, template defaults, broad legacy profiles, or
overlays.

Schema rules:

- Deployment config accepts only app-infrastructure family keys.
- Capsule templates accept only capsule family defaults/fixed/open parts.
- Singleton families accept one provider id. Multi-provider families must declare cardinality in their family
  contract before a selection surface can select more than one.
- Provider-owned config is a map from provider id to config object, validated against that provider's schema.
- Secret-bearing config uses object refs such as `{ secretRef: "slack-bot-token" }`; literal secret values and
  ad-hoc `secret:*` strings are rejected.
- Unknown family keys, provider ids that do not register in `ProviderRegistry`, disabled security-critical families,
  duplicate selections, and unresolved config refs reject the deployment/template selection.

Example split default:

```yaml
apiVersion: gla.dev/v1
kind: AppDeploymentConfig
metadata:
  name: local-dev
spec:
  select:
...
metadata:
  name: browser-handoff-reference
spec:
  defaults:
...
Operators or their agents should extend by candidate inventory/deployment config and template package updates, not by
modifying generic app composition:
...
A deployment/template update may select registered provider ids and supply provider-owned config refs in the owner
surface. It must not alter core authorization semantics, inject executable provider code at runtime, bypass WPM
dependency evidence, or introduce provider-specific imports into app/kernel/gateway/session/identity/worker packages.
...
"best" provider search. Deployment config and templates select concrete ids; compatibility and availability validate
them.
...
8. Admission Resolver resolves an `AssemblySpec` only against this validated graph projection. It never reimplements
   a second availability or compatibility rule.

Config/default precedence is fixed from lowest to highest:

1. Provider `config_schema` defaults.
2. Compatibility `defaultConfig` used only to normalize legacy distribution inputs.
3. `AppDeploymentConfig` provider config for app-infrastructure providers.
4. `TemplatePackage` / `CapsuleTemplate` defaults for selected capsule parts and part params.
5. Runtime `AssemblySpec` part params where the template and family contract permit runtime selection.

WPM `DependencyBinding` evidence is not ordinary config and does not participate in this merge. ProviderRegistry and
its internal factory runner pass accepted connection refs to provider factories through `ProviderCreateContext.dependencies`. Template-fixed
...
App composition must supply the template reachability probe through explicit composition options or provider/template
probe descriptors. If a template or provider declares a probe that the app graph cannot register, the graph treats
that current probe as `unavailable`; accepted WPM receipt evidence is not enough by itself.
...
Provider-owned browser assets are served through generic gateway asset mounts only after catalog/runtime provenance
...
Provider state lives in ProviderRegistry/internal ProviderHost namespaces and follows daemon-state recovery rules:
...
| Operator install/update | How does an operator or operator agent choose, configure, verify, and roll back providers on a concrete host? | active provider inventory, `AppDeploymentConfig`, capsule defaults, WPM dependency receipts, daemon config, doctor evidence. |
...
runtime consumption cannot write provider code, deployment config, template defaults, WPM receipts, or daemon config.
...
5. **Produce a readiness handoff** for operator install/update: package id/version/family, manifest/schema/probe
   summary, skills/docs, test evidence, dependency requirements, optional WPM bundle refs, and deployment/template
   staging notes.

The operator install/update flow starts from that readiness handoff. It adds the package to a trusted registry
bootstrap inventory, selects it through `AppDeploymentConfig` or capsule defaults, runs WPM for host-touching
dependencies, records `DependencyBinding` evidence, restarts or boots the daemon, and runs doctor so registry
registration, Catalog availability, and current probes are verified.
...
deployment config, template defaults, write WPM receipts, or load executable provider code.

The current executable command leaves for these flows are provider authoring (`gla provider scaffold`,
`gla provider validate`, `gla provider test`, `gla provider inspect`), template package authoring
(`gla template-package scaffold`, `gla template-package validate`, `gla template-package test`,
`gla template-package inspect`), operator install/update (`gla provider-install plan`,
`gla provider-install apply`, `gla provider-install rollback`), and doctor (`gla doctor provider-graph`). Future
convenience commands must remain wrappers over the contract above, not new provider-layer owners.

## 10. Validation Stages

| Stage | What is checked | Failure result |
|---|---|---|
| Build/registry validation | manifests, schemas, duplicate ids, family registration, contract tests, skills/docs, boundary imports | registry bootstrap cannot pass the quality gate. |
| Deployment/template validation | owner-specific family keys, provider ids, cardinality, secret refs, config schemas, compatibility declarations | deployment config or template package cannot be applied. |
...
| Boot validation | provider modules register, deployment/template selections exist, config/state schemas validate, probes run, diagnostics redact | daemon fails closed or provider is unavailable. |
...
| Admission Resolver validation | template/assembly subgraph resolves to available compatible providers and policy allows it | proposal rejected before task/session/route/capsule creation. |
...
1. **Canonicalize graph types.** Add explicit graph vocabulary to ProviderRegistry/Catalog types: family, provider,
   dependency, registry, deployment config, template, probe, skill, asset, state, and compatibility descriptors.
2. **Separate template packages from runtime providers.** `CapsuleTemplate` packaging gets catalog/schema/tests and
   compatibility requirements, but no executable provider factory requirement.
...
4. **Deployment config and template default schema.** Add validation for canonical family ids, provider-id
   resolution, unknown keys, disabled/null providers, duplicate selection, config refs, and restart/apply lifecycle.
5. **Graph resolver.** Implement one resolver used by Catalog, Doctor, ProviderRegistry boot, and Admission Resolver.
   It owns compatibility, availability, and config-default precedence.
...
- unknown provider id selected by deployment config or template defaults is rejected;
- unregistered provider selected by an operator inventory is rejected;
- legacy profile/overlay inheritance cycles are rejected while compatibility readers remain;
...
- literal secret values and ad-hoc secret strings in deployment/provider config are rejected;
...
- adding a new provider in a migrated family changes only the provider package, registry bootstrap/inventory,
  `AppDeploymentConfig` or template defaults, WPM bundle/receipt if needed, and tests/docs;
...
- a fake provider in each prioritized family can be selected by deployment config or capsule defaults and exercised
  through catalog, schema, factory creation, doctor/probe, admission, and the relevant runtime path;
...
| Concrete candidates and reference alternatives | `docs/03-software-candidates.md`; deployment/template default wording must name the preset |
| Target provider-layer split | `docs/architecture/provider-layer-refactor-contract.md` |
| Internal ProviderHost mechanics and legacy provider-set/profile compatibility history | `docs/architecture/provider-host-extension-architecture.md` |
| Provider contribution rules | `docs/architecture/provider-author-workflow.md` plus this graph plan's workflow rules |
