...
GLA on `:3000` inside LXD `hermes-1`, fronted by host Caddy at `https://203.0.113.10/`.
...
- **Human door = Access Gateway** (`gateway`): the **only** publicly reachable surface; every public request
  ...
  **Caddy** is the TLS edge in *front* of this — Caddy terminates TLS at `https://203.0.113.10/` and proxies
...
3. **Caddy as host TLS edge *in front of* the Access Gateway.** `docs/03 §3` casts Caddy as *the* public edge
   ...
   `203.0.113.10` and **GLA's Access Gateway** remains the grant-verifying PEP on `:3000` behind it. This is a
