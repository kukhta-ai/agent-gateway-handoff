...
|  | `gla catalog show <id>` | 1. Read one provider/template catalog entity from the active graph<br>2. Print family, capability, system-derived availability, config schema, dependency evidence, provenance, selected-default source, compatible providers/template parts, and diagnostics<br>3. Exit 5 if unknown<br>4. *Read-only* |
