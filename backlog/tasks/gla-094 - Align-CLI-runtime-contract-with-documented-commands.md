...
<!-- SECTION:NOTES:BEGIN -->
Created from independent consistency review. Evidence: docs/05-cli-and-entities.md command contract and surfaces/cli/src/cli.ts help/runtime parser diverge. This should be closed as a public-surface contract issue, not left as incidental docs cleanup.

Scope validation update: docs/05 currently promises more than the runtime implements. Concrete current gaps include missing schema command; missing policy/events/audit/auth nouns; no --fields implementation despite scenario examples using it; no ndjson output mode; no --context, --no-input, or --trace-id globals; root-only help payload instead of command-scoped machine help; --endpoint exists in runtime but is not part of docs/05 globals; daemon/in-process connection behavior is not described consistently; version currently reports only client. GLA-094 should reconcile these as a current-surface contract task, not as a broad mandate to implement every future CLI noun.

User clarification: GLA-094 must not erase future development plans or CLI vision from docs/05. The intended fix is separation and labeling: current supported contract versus deferred/future contract. Future-facing nouns, flags, MCP parity, agent-first principles, and roadmap content should remain visible unless separately superseded by an approved design change.
<!-- SECTION:NOTES:END -->
