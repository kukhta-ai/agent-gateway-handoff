...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 A documented test layout distinguishes runtime source, package-local unit tests, package-local contract tests, app integration/E2E tests, package fixtures, and top-level cross-cutting scenario or boundary tests.
- [ ] #2 The quality gate typechecks tests that live outside src with the same strictness as current src-co-located tests.
- [ ] #3 Vitest discovers the documented test locations without requiring test files to live under src.
- [ ] #4 Production package builds and emitted dist artifacts exclude tests and fixtures while preserving test typechecking in the gate.
- [ ] #5 The documented convention explains how provider packages ship contract tests with provider code without placing runtime tests in src.
- [ ] #6 The migration path preserves current test behavior and does not weaken browser-E2E preflight or existing boundary checks.
<!-- AC:END -->
